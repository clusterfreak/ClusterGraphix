...
public class ClusterGraphix extends JPanel implements ActionListener {
	...
	private String[] pixelObjectMembershipColHeads = { "#", "1", "2" };
	private JTable pixelObjectMembershipTable = new JTable();
	...
	private final void clusterGraphixGenerator() {
		...
		clusterMenuViewClusterCircle.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent e) {
				setClusterCircle(clusterMenuViewClusterCircle.isSelected());
			}
		});
		...
		try {
			fValidate.setIconImage(ImageIO.read(getClass().getResource("/de/clusterfreak/ClusterCore/sphere32.png")));
		} catch (Exception e) {
			JOptionPane.showConfirmDialog(null, e, "ClusterGraphix.setIconImage", JOptionPane.CLOSED_OPTION,
					JOptionPane.INFORMATION_MESSAGE);
		}
		...
		try {
			fData.setIconImage(ImageIO.read(getClass().getResource("/de/clusterfreak/ClusterCore/sphere32.png")));
		} catch (Exception e) {
			JOptionPane.showConfirmDialog(null, e, "ClusterGraphix.setIconImage", JOptionPane.CLOSED_OPTION,
					JOptionPane.INFORMATION_MESSAGE);
		}
		...
		try {
			fCheck.setIconImage(ImageIO.read(getClass().getResource("/de/clusterfreak/ClusterCore/sphere32.png")));
		} catch (Exception e) {
			JOptionPane.showConfirmDialog(null, e, "ClusterGraphix.setIconImage", JOptionPane.CLOSED_OPTION,
					JOptionPane.INFORMATION_MESSAGE);
		}
		...
		try {
			fInfo.setIconImage(ImageIO.read(getClass().getResource("/de/clusterfreak/ClusterCore/sphere32.png")));
		} catch (Exception e) {
			JOptionPane.showConfirmDialog(null, e, "ClusterGraphix.setIconImage", JOptionPane.CLOSED_OPTION,
					JOptionPane.INFORMATION_MESSAGE);
		}
	...
	public void actionPerformed(ActionEvent ev) {
		...
		if ("Show".equals(actionCommand)) {
			...
			if (clusterFile.getData(39))
				miscData[39][3] = "[" + String.valueOf(getPixelObjectMembership().length) + "]";
			else
				miscData[39][3] = "[null]";
			...
			if (getPixelObjectMembership() != null) {
				...
				pixelObjectMembershipColHeads = new String[getPixelObjectMembership().length + 1];
				...
				pixelObjectMembershipTable = null;
			// pixelVi
			if (getPixelVi() != null) {
								...
								try {
									if (getPixelObject()[i][k] == true) {
										...
										if (clusterFile.getData("pixelObjectMembership")) {
											if (getPixelObjectMembership()[i][k] == false)
												g2.setColor(Color.black);
										}
										if ((int) (pixelPaint * getZoom()) < 1)
											g2.fillRect((int) ((i) * pixelPaint * getZoom()),
	...
	private void setPixelObjectMembership(boolean pixelObjectMembership[][]) {
		...
		if (pixelObjectMembership == null)
			...
			clusterFile.setData("PixelObjectMembership", true);
	}
	...
	private void setClusterCircle(boolean clusterCircle) {
		...
		if (clusterCircle == false)
			clusterFile.setData("ClusterCircle", true);
		else
			clusterFile.setData("ClusterCircle", false);
	}
	 ...
	 * 
	 * @param force
	 *            overwrites when pixel original
	 */
	...
	*/
	...
	private void info() throws IOException {
		...
		infoLabel1.setText("ClusterGraphix " + version + " (" + String.valueOf(date.getYear() + 1900) + "-"
